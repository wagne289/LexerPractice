...
"""
Creates tokens from certain words in the program(string) for the parser

Parameter: program(str): string of the program wanting to be run
"""
def lex_gobbledegook(program):
    lexgen = LexerGenerator()

    lexgen.add('CONSOLESAYS', r'consolesays')
    lexgen.add('SEMICOLON', r';')
    lexgen.add('AND', r'and')
    lexgen.add('OR', r'or')
    lexgen.add('IF', r'if')
    lexgen.add('ELSE', r'else')
    lexgen.add('START', r'start')
    lexgen.add('END', r'end')
...
"""
Takes the generated lexer and establishes rules for certain sets

Parameter: program(str): string of the program wanting to be run
"""
def parse_gobbledegook(program):
    ...
    @pg.production('scope : START SEMICOLON statements END SEMICOLON')
    ...
    @pg.production('statement : CONSOLESAYS expression SEMICOLON')
    def sparty_Statement(state, p):
        state.id += 1
        return {
            "id": state.id,
            "type": "statement",
            "statement_type": "consolesays",
            "expression": p[1]
        }
...
"""
Converts the provided string into a runnable program

Parameter: program(str): string of the program wanting to be run
"""
def interpret_gobbledegook(program):

    ir = parse_gobbledegook(program)
    ...
    #use to handle 'strings' inside the program
    ...
    # use to handle 'int' inside the program
    ...
    def interpret_scope(ir, rts):
        ...
        while True:
            ...
            if statement["statement_type"] == "consolesays":
